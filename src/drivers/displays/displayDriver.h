...
typedef int  (*AlternateFunction)(void);
...
typedef void (*ScreenBrightnessFunction)(char value);

typedef struct
{
  DriverInitFunction initDisplay;                    // Initialize the display
  ScreenBrightnessFunction setBrightness;            // Screen brightness level
  ...
  int screenOn;    // TFT_BACKLIGHT_ON;              // screen on state
