...
bool reset_configuration(int sinceMs, int interval) {
...
void init_WifiManager()
{
    ...
    wm.setConfigPortalTimeout(5*50); // auto close configportal after n seconds
    ...
    // Text box (String) - 80 characters maximum
    WiFiManagerParameter addr_text_box("btcAddress", "Your BTC address", Settings.BtcWallet.c_str(), 80);
  ...
  // Text box (String) - 80 characters maximum
  WiFiManagerParameter password_text_box("Poolpassword - Optional", "Pool password", Settings.PoolPassword.c_str(), 80);
  ...
  wm.addParameter(&invertColors);
    ...
    if (forceConfig)
    {
        ...
        if (!wm.startConfigPortal(DEFAULT_SSID, DEFAULT_WIFIPW))
        {
            ...
            Settings.PoolPassword= password_text_box.getValue();
            Settings.BtcWallet = addr_text_box.getValue();
